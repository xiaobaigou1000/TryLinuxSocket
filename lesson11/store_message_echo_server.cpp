...
int main()
{
    ...
    int fileWritePipes[2];
    pipe(fileWritePipes);
    pid_t fileWritePid = fork();
    if (fileWritePid == 0)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            int len = read(fileWritePipes[0], msgbuf, 512);
            fwrite(msgbuf, len, 1, fp);
        }
        fclose(fp);
        return 0;
    }

    auto handleConnection = [server_socket, writeToPipe = fileWritePipes[1]](int client_socket) {
        close(server_socket);
        int strLength;
        ...
        while ((strLength = read(client_socket, buffer.data(), buffer.size())) != 0)
        {
            ...
            write(writeToPipe, buffer.data(), strLength);
        }
